...
@Injectable()
export class UserProspectiveInfoResolver extends AbstractResolver<PageNavigationData> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<PageNavigationData | string> {
    if (SECURE_LS && SECURE_LS.get('currentUser')) {
      ...
      return this.listingUserRelationService.getProspectiveListings(currentUser.userId)
        .pipe(
