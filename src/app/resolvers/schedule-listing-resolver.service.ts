...
@Injectable()
export class ScheduleListingResolver extends AbstractResolver<PageNavigationData> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<PageNavigationData | string> {
    if (SECURE_LS && SECURE_LS.get('currentUser')) {
      ...
      return this.listingUserRelationService.getUserRelatedListings(currentUser.userId)
        .pipe(
